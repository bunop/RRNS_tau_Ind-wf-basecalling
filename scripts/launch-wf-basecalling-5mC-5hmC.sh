...
#SBATCH --mem=16G
#SBATCH --error=basecalling-5mC-5hmC.err
#SBATCH --output=basecalling-5mC-5hmC.out
#SBATCH --job-name=basecalling-5mC-5hmC
...
nextflow run ${WORK}/wf-basecalling \
    --custom_config_base ${CUSTOM_CONFIG_BASE} \
    -config ${CUSTOM_CONFIG_BASE}/nfcore_custom.config \
    -config conf/custom-wf-basecalling.config \
    -profile ibba,galileo,singularity -resume -params-file conf/params-wf-basecalling-5mC_5hmC.json
